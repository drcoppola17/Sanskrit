      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="grid gap-2">
          {targets.map(k => {
            ...
            return (
              <div key={k}
                   className={`p-3 rounded-2xl border ${isDone ? "bg-emerald-50 dark:bg-emerald-900/40" : "bg-zinc-50 dark:bg-zinc-900"}`}
          ...
          {pairs.map(k => {
            ...
            return (
              <div key={k}
                   ...
                   className={`p-3 rounded-2xl shadow select-none ${isDone ? "opacity-40" : "cursor-pointer bg-white dark:bg-zinc-800"} ${isSel && !isDone ? "ring-2 ring-indigo-500" : ""}`}>
                {pose.sa}
              </div>
            );
          })}
        ...
        {cards.map((c, idx) => {
          ...
          return (
            <div key={idx} onClick={() => click(idx)}
                 className={`p-3 rounded-2xl h-20 flex items-center justify-center shadow cursor-pointer ${show ? "bg-white dark:bg-zinc-800" : "bg-zinc-200 dark:bg-zinc-700"}`}>
      ...
      <div className="max-w-4xl mx-auto grid gap-6">
        <header className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            {MODES.map(m => (
              <button key={m.id}
                className={`px-3 py-2 rounded-2xl shadow flex items-center gap-2 ${mode===m.id?"bg-indigo-200 dark:bg-indigo-800":"bg-white dark:bg-zinc-800"}`}
